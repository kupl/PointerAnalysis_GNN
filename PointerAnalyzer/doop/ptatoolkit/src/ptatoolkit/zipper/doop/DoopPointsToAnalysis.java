...
import static ptatoolkit.zipper.doop.Attribute.METHODS_CALLED_ON;
import static ptatoolkit.zipper.doop.Attribute.OBJECT_ASSIGNED;
import static ptatoolkit.zipper.doop.Attribute.POINTS_TO_SET;
...
import static ptatoolkit.zipper.doop.Attribute.TYPE;
import static ptatoolkit.zipper.doop.Attribute.VARS_IN;

public class DoopPointsToAnalysis implements PointsToAnalysis {

    protected static final String ARR_FIELD = "@ARRAY";
    protected final DataBase db;
    protected Set<Obj> allObjs;
    protected int totalPTSSize;
    protected Set<Method> reachableMethods;
    protected Set<Method> applicationMethods;
    // The following factories may be used by iterators
    protected VariableFactory varFactory;
    protected ObjFactory objFactory;
    protected FieldFactory fieldFactory;
    protected Set<String> specialObjects;
    protected Map<Type, Type> directSuperType;
    protected Map<Type, Set<Obj>> typeObjects;
    protected Map<Type, Set<Method>> typeMethods;
    protected List<Pair<Variable, Variable>> thisAssign;
    protected Map<Type, Set<Type>> assignableTypes;
    ...
    public Set<Method> reachableMethods() {
        // These reachable methods only contain the methods that have
        // caller or callee. Some implicit reachable methods are not included.
        return reachableMethods;
    }

    @Override
    public Set<Method> applicationMethods() {
    ...
    @Override
    public Type typeOf(Variable var) {
        return (Type) var.getAttribute(TYPE);
    }
    ...
    public Iterator<Pair<Variable, Variable>> localAssignIterator() {
        return new Iterator<Pair<Variable, Variable>>() {
            ...
            private final Iterator<List<String>> assignIter =
    ...
    public Iterator<Triple<Variable, Obj, Field>> instanceLoadIterator() {
        return new Iterator<Triple<Variable, Obj, Field>>() {
            ...
            private final Iterator<List<String>> loadIter =
                    db.query(Query.INSTANCE_LOAD);
            private final Iterator<List<String>> arrayLoadIter =
    ...
    public Iterator<Triple<Obj, Field, Variable>> instanceStoreIterator() {
        return new Iterator<Triple<Obj, Field, Variable>>() {
            ...
            private final Iterator<List<String>> storeIter =
                    db.query(Query.INSTANCE_STORE);
            private final Iterator<List<String>> arrayStoreIter =
    ...
    @Override
    public Set<Type> assignableTypesOf(Type type) {
        return assignableTypes.get(type);
    }
    ...
    protected void init() {
        ...
        db.query(Query.INST_METHODS).forEachRemaining(list -> {
            ...
            Stream.concat(instMtd.getParameters().stream(),
        ...
        Iterator<List<String>> iterator = db.query(Query.SPECIAL_OBJECTS);
        Iterable<List<String>> iterable = () -> iterator;
        specialObjects = StreamSupport.stream(iterable.spliterator(), false)
                .map(list -> list.get(0))
        ...
        // build type-related information
        buildDeclaringType(mtdFactory, typeFactory);
        buildDirectSuperType(typeFactory);
        buildAssignableType(typeFactory);
        buildVariableType(varFactory, typeFactory);

        applicationMethods = new HashSet<>();
        db.query(Query.APP_METHODS).forEachRemaining(list -> {
            String mtdSig = list.get(0);
            applicationMethods.add(mtdFactory.get(mtdSig));
        });
    ...
    protected void buildPointsToSet(VariableFactory varFactory, ObjFactory objFactory,
    ...
    protected void computeAllocatedObjects(ObjFactory objFactory,
    ...
    protected void buildMethodsInvokedOnObjects(MethodFactory mtdFactory) {
     ...
     */
    protected void buildReceiverObjects() {
        for (Obj obj : allObjects()) {
            for (Method method : methodsInvokedOn(obj)) {
                method.addToAttributeSet(Attribute.RECEIVER, obj);
            }
        }
    }

    protected void buildVarDeclaringMethods(VariableFactory varFactory,
                                            MethodFactory mtdFactory) {
    ...
    protected void buildObjectAssignedVariables(ObjFactory objFactory,
    ...
    protected void buildDeclaringType(MethodFactory mtdFactory,
    ...
    protected void buildDirectSuperType(TypeFactory typeFactory) {
    ...
    protected void buildAssignableType(TypeFactory typeFactory) {
        assignableTypes = new HashMap<>();
        db.query(Query.ASSIGN_COMPATIBLE).forEachRemaining(list -> {
            Type target = typeFactory.get(list.get(0));
            Type source = typeFactory.get(list.get(1));
            if (!assignableTypes.containsKey(target)) {
                assignableTypes.put(target, new HashSet<>());
            }
            assignableTypes.get(target).add(source);
        });
    }

    protected void buildVariableType(VariableFactory varFactory, TypeFactory typeFactory) {
        db.query(Query.VAR_TYPE).forEachRemaining(list -> {
            Variable var = varFactory.get(list.get(0));
