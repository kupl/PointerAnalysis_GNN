...
import static soot.util.dot.DotGraphConstants.NODE_SHAPE_BOX;
import static soot.util.dot.DotGraphConstants.NODE_SHAPE_ELLIPSE;
import static soot.util.dot.DotGraphConstants.NODE_STYLE_DOTTED;
...
public class Dumper {
    ...
    private static DotGraph drawObjectFlowGraph(IObjectFlowGraph g) {
        ...
        g.allNodes().forEach(node -> {
            ...
            g.outEdgesOf(node).forEach(edge -> drawEdge(canvas, edge));
        });
        return canvas;
    }
