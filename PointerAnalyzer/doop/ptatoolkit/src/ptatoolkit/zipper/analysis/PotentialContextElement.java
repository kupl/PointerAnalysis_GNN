...
public class PotentialContextElement {
    ...
    private final Map<Type, Set<Method>> typePCEMethods = new HashMap<>();
    ...
    public PotentialContextElement(PointsToAnalysis pta, ObjectAllocationGraph oag) {
        this.pta = pta;
        init(oag);
    }
    ...
    public Set<Method> PCEMethodsOf(Obj obj) {
        return obj.getAttributeSet(Attr.PCE);
    }
    ...
    public Set<Obj> PCEObjectsOf(Method method) {
        return method.getAttributeSet(Attr.PCE);
    }
    ...
    private void init(ObjectAllocationGraph oag) {
        ...
        topoSorter.sort(mg, true).forEach(node -> {
            ...
            node.getContent().forEach(obj -> obj.setAttribute(Attr.PCE, methods));
        });
        invokedMethods = null;
        if (Global.isDebug()) {
            computePCEObjects();
    ...
    private void computePCEObjects() {
        ...
        pta.allObjects()
                .forEach(obj -> PCEMethodsOf(obj).forEach(method -> {
        ...
        pceObjs.forEach(((method, objs) ->
                method.setAttribute(Attr.PCE, setFactory.get(objs))));
    }
