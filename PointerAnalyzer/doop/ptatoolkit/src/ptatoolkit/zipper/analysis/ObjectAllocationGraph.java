...
public class ObjectAllocationGraph implements DirectedGraph<Obj> {
    ...
    public ObjectAllocationGraph(PointsToAnalysis pta) {
        this.pta = pta;
        init();
    }
    ...
    public Set<Obj> allocateesOf(Obj obj) {
        return obj.getAttributeSet(Attr.ALLOCATEES);
    }
    ...
    private void computeAllocatees() {
        ...
        sorter.sort(mg, true).forEach(node -> {
            ...
            node.getContent()
                    .forEach(obj -> obj.setAttribute(Attr.ALLOCATEES, allocatees));
        });
    }
