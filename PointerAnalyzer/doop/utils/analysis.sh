    ...
    do
        ...
        if [[ -e $cachefacts ]] && flag-isset cache; then
            ...
            if flag-isset averroes; then
                ...
                linked="$( ${DOOP_HOME}/bin/jre-link-arguments $( meta-load jre ) ) "
                for dep in $( meta-load deps ); do
                    linked="$linked -l $dep"
                done
            fi
            ...
            if flag-isset dump; then
                sootFactGenArgs="$sootFactGenArgs -dump"
            fi
            ...
            timing java -cp ${SOOT_CLASSES}:${SOOT_FACT_GEN} Main \
                $sootFactGenArgs -d $DOOP_HOME/tmp/facts $injar $inputjars
...
EOF
    ...
    # Read context-sensitive methods
	if [ ! "x$csmethod" = "x" ]; then
		printf "loading ${C_YELLOW}context-sensitive methods${C_RESET} from %s " $csmethod
		#echo "loading context-sensitive methods ... "

                                cat > tmp/SuspectInvo.import <<EOF
option,delimiter,"\t"
option,hasColumnNames,false

fromFile,"/home/minseokjeon/Downloads/Baton/doop/SuspectInvo.facts",i,invo
toPredicate,SuspectInvo,invo
EOF
                $bloxbatch -db $database -import $(pwd)/tmp/SuspectInvo.import

		cat > tmp/csmethod.import <<EOF
option,delimiter,"	"
option,hasColumnNames,false

fromFile,"$(readlink -f $csmethod)",a,csmethod
toPredicate,ContextSensitiveMethod,csmethod
EOF
		timing $bloxbatch -db $database -import $(pwd)/tmp/csmethod.import
	fi

    # Read precise var points-to relations
	if [ ! "x$precisevpt" = "x" ]; then
		printf "loading ${C_YELLOW}precise var points-to${C_RESET} from %s " $precisevpt
		cat > tmp/precisevpt.import <<EOF
option,delimiter,"	"
option,hasColumnNames,false

fromFile,"$(readlink -f $precisevpt)",a,obj,b,var
toPredicate,PreciseVarPointsTo,obj,var
EOF
		timing $bloxbatch -db $database -import $(pwd)/tmp/precisevpt.import
	fi

        # Read specified context variant relations
	if [ ! "x$specifiedcs" = "x" ]; then
		printf "loading ${C_YELLOW}specified context variants${C_RESET} from %s " $specifiedcs
		cat > tmp/specifiedcs.import <<EOF
option,delimiter,"	"
option,hasColumnNames,false

fromFile,"$(readlink -f $specifiedcs)",a,method,b,ctx
toPredicate,SpecifiedContextVariant,method,ctx
EOF
		timing $bloxbatch -db $database -import $(pwd)/tmp/specifiedcs.import
	fi
...
fromFile,"$(readlink -f tmp/ClassNewInstance.facts)",a,invo,b,type
...
fromFile,"$(readlink -f tmp/ConstructorNewInstance.facts)",a,invo,b,constructor
...
fromFile,"$(readlink -f tmp/MethodInvoke.facts)",a,invo,b,method
