...
def convertReflectionLog(refLog, factsDir, outDir):
 ...
 outputDict = {
  #'Class.forName':open(os.path.join(outDir, 'ClassForName.facts'), 'w'),
  'Class.newInstance':open(os.path.join(outDir, 'ClassNewInstance.facts'), 'w'),
  'Constructor.newInstance':open(os.path.join(outDir, 'ConstructorNewInstance.facts'), 'w'),
  #'Field.get*':open(os.path.join(outDir, 'FieldGet.facts'), 'w'),
  #'Field.set*':open(os.path.join(outDir, 'FieldSet.facts'), 'w'),
  'Method.invoke':open(os.path.join(outDir, 'MethodInvoke.facts'), 'w'),
 }
 for line in flog:
